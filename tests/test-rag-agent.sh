...
function test_rag_agent {
  echo "===> test_rag_agent: start"

  if uv run python tests/test-rag-agent.py; then
    echo "===> test_rag_agent: pass"
    return 0
  else
    echo "===> test_rag_agent: fail"
